// HomePage.js - 首页（Today's Trending）
 ...
 */
export function renderHomePage(newsData = []) {
  // 生成新闻卡片HTML
  const newsCards = newsData.map(renderNewsCard).join('');
  
  // 生成分类标签HTML
  const categories = ['All', 'Politics', 'Economy', 'Technology', 'Society', 'Entertainment', 'Health', 'Education', 'Environment', 'Finance'];
  const categoryTabs = renderCategoryTabs(categories, 'All');
    ...
    <main class="main-content">
      ...
      <section class="news-section">
        <div class="section-container">
          ...
          <div class="load-more">
            <button class="btn btn-outline" id="load-more-button">Load More</button>
          </div>
        </div>
      </section>
    </main>
...
export function initHomePage(options = {}) {
  ...
  // 初始化分类标签交互
  initCategoryTabs(onCategoryChange);
